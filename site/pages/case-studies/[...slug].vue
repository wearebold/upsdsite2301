...
const { data } = await useAsyncData(`content-${path}`, async () => {
  ...
  let surround = queryContent('case-studies').only(["_path", "title", "description"]).sort({ date: 1 }).findSurround(path);
  ...
  <main id="main" class="article-main">
    <header v-if="data.article" class="article-header">
      <div class="img-cont h-72 mb-12">
        <img :src="`/img/case-studies/${data.article.img}`" :alt="data.article.title" class="" />
      </div>
